...
class GenBeeE2ETrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            self.model = GenerativeModel(self.config, self.tokenizer, self.type_set)
    ...
    def train(self, train_data, dev_data, **kwargs):
        ...
        for epoch in range(1, self.config.max_epoch+1):
            ...
            optimizer.zero_grad()
            cummulate_loss = []
            for batch_idx, batch in enumerate(DataLoader(internal_train_data, batch_size=self.config.train_batch_size // self.config.accumulate_step, shuffle=True, drop_last=False, collate_fn=ED_collate_fn)):
            ...
            if epoch in [1,  10, 20,  30, 40] or epoch > 42:
                # eval dev
                ...
                if dev_scores["argument_cls"]["f1"] >= best_scores["argument_cls"]["f1"]:
                    ...
                    state = dict(model=self.model.state_dict(), tokenizer=self.tokenizer, type_set=self.type_set ) # save  prefix-roberta, ffnn state
    ...
    def internal_eval(self, eval_data, split="Dev"):
        ...
        print(self.config.eval_batch_size)
    ...
    def predict(self, data, **kwargs):
        ...
        predictions = []
        for batch_idx, batch in enumerate(DataLoader(eval_data, batch_size=self.config.eval_batch_size, 
                                                     shuffle=False, collate_fn=ED_collate_fn_test)):
            progress.update(1)
            ...
            for event_type in self.vocab['event_type_itos']:
                ...
                pred_texts = self.model.generate(enc_idxs, enc_attn, event_type_list, num_beams=self.config.beam_size, max_length=self.config.max_output_length)
